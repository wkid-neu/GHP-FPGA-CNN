...
int main(int argc, char const *argv[]) {
    ...
    while ((cmd_opt = getopt_long(
        argc, (char * const*)argv, "0:1:2:3:4:", long_opts, NULL
    )) != -1) {
        switch (cmd_opt)
        {
        case 0:
...
void run(
    ...
    /* Run the test */
    uint32_t latency;
    test_fc1(h2c_fd, csr_map_base, im_d2c_intr_fd, exec_intr_fd, OC, INC, t_mode, &latency);
    /* Display test results */
    printf("OC: %u, INC: %u, t_mode: %d, Latency: %u cycles, Time(ns): %u\n", OC, INC, t_mode, latency, LATENCY_NS(latency));
    /* Close devices */
    close(csr_fd);
    close(h2c_fd);
