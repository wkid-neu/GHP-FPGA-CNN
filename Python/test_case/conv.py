...
import shutil
import math
from fe.op_QLinearConv import make_hw_graph, exec_graph
...
def parse_testcase_conf(args) -> dict:
    ...
    if not os.path.exists(fp):
        ...
        testcase.update({"w_mode": str(args.w_mode)})
...
def gen(
    ...
    for case_idx in range(len(testcase_conf["cases"])):
        ...
        ori_w = np.random.randn(OC, INC, KH, KW)
        ...
        # Save original tensors for debugging
        np.savetxt(os.path.join(output_dir_path, f"dbg_x{case_idx}.txt"), x.flatten(), fmt="%d")
        np.savetxt(os.path.join(output_dir_path, f"dbg_w{case_idx}.txt"), w.flatten(), fmt="%d")
        np.savetxt(os.path.join(output_dir_path, f"dbg_B{case_idx}.txt"), B.flatten(), fmt="%d")

        # Regenerate B
        B = base.conv_regen_bias(B, w, int(x_zero_point), int(w_zero_point))
        np.savetxt(os.path.join(output_dir_path, f"dbg_new_B{case_idx}.txt"), B.flatten(), fmt="%d")

        # Weights and bias reordering
        reordered_w = base.conv_reorder_weights(w, M, S)
        reordered_B = base.conv_reorder_bias(B, M).astype(np.int32)
        ...
        # Generate the synchronized matrix
        sync_mat = []
        n_w_rnd = OC//(M*2)
        vec_size = INC*KH*KW//2
        for i in range(n_x_rnd):
            subx = mat_X[i*vec_size:(i+1)*vec_size, :]
            for j in range(n_w_rnd):
                subw = reordered_w[j*vec_size:(j+1)*vec_size, :]
                joint = np.column_stack((subx, subw))
                sync_mat.append(joint)
        sync_mat = np.row_stack(sync_mat)
        ...
        mat_X.tofile(os.path.join(output_dir_path, f"matx{case_idx}.hex"))
        reordered_w.tofile(os.path.join(output_dir_path, f"w{case_idx}.hex"))
        reordered_B.tofile(os.path.join(output_dir_path, f"B{case_idx}.hex"))
        sync_mat.tofile(os.path.join(output_dir_path, f"sync_mat{case_idx}.hex"))
        ...
        rtm_addr += Y_len
        if w_mode=="dyn":
            W_addr, W_n_bytes = ddr_addr, OC*INC*KH*KW
            ddr_addr += W_n_bytes
        else:  # w_mode=="sta"
            W_addr, W_len = cwm_addr, OC*INC*KH*KW//(4*M)
            cwm_addr += W_len
        B_addr, B_len = bm_addr, OC//16
        bm_addr += B_len
        instructions.append((
            xphs_addr, n_x_rnd-1, W_addr, OC*INC*KH*KW, B_addr, X_addr, Y_addr,
            OC, INC//S-1, INW_, KH-1, KW-1, strideH, strideW, padL, padU,
            INH2, INW2, ifm_height, ofm_height, n_last_batch, n_w_rnd-1, row_bound, col_bound, vec_size, vec_size_minus_1,
            int(x_zero_point), int(w_zero_point), int(y_zero_point), m1, n1-1,
            cwm_addr & 0x000000ff, (cwm_addr & 0x0000ff00) >> 8, (cwm_addr & 0x00ff0000) >> 16, (cwm_addr & 0xff000000) >> 24
        ))
