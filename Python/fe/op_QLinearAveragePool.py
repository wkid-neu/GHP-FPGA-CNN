...
def hw_impl(
    ...
    fake_weight = np.ones((INC, 1, KH, KW))
    fake_bias = np.zeros((INC,))
    pads = np.array([0, 0, padU, padL, 0, 0, padD, padR])
    tensors.extend([
        helper.make_tensor(name=f"{name}/Constant_0", data_type=TensorProto.FLOAT, dims=fake_weight.shape, vals=fake_weight.flatten()),
        helper.make_tensor(name=f"{name}/Constant_1", data_type=TensorProto.FLOAT, dims=fake_bias.shape, vals=fake_bias.flatten()),
        helper.make_tensor(name=f"{name}/Constant_2", data_type=TensorProto.INT64, dims=[], vals=[nxz]),
        ...
        helper.make_tensor(name=f"{name}/Constant_5", data_type=TensorProto.INT64, dims=[], vals=[yz]),
        ...
        helper.make_tensor(name=f"{name}/Constant_8", data_type=TensorProto.INT64, dims=[], vals=[xz]),
        helper.make_tensor(name=f"{name}/Constant_9", data_type=TensorProto.INT64, dims=pads.shape, vals=pads.flatten()),
    ])
    ...
    nodes.append(helper.make_node(
        op_type="Pad",
        inputs=[f"{name}/Cast_0", f"{name}/Constant_9", f"{name}/Constant_8"],
    ...
    nodes.append(helper.make_node(
        op_type="Conv",
        inputs=[f"{name}/Cast_1", f"{name}/Constant_0", f"{name}/Constant_1"],
    ...
    nodes.append(helper.make_node(
        op_type="Sub",
        inputs=[f"{name}/Cast_2", f"{name}/Constant_2"],
    ...
    nodes.append(helper.make_node(
        op_type="Add",
        inputs=[f"{name}/hw_shift_0", f"{name}/Constant_5"],
    ...
    nodes.append(helper.make_node(
        op_type="Clip",
        inputs=[f"{name}/Add_0", f"{name}/Constant_6", f"{name}/Constant_7"],
...
def make_sw_graph(
    ...
    node = helper.make_node(
        ...
        ceil_mode=ceil_mode,
        channels_last=0,
        count_include_pad=1,  # only this mode is supported
...
def make_hw_graph(
    ...
    # prepare parameters
    N = kernel_shape[0]*kernel_shape[1]
    ...
    params = {
        ...
        "m1": m1, "n1": n1,
        "xz": int(x_zero_point), "nxz": int(x_zero_point)*N, "yz": int(y_zero_point)
    }
